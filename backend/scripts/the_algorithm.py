import sys
import p_extract as pvf # Pochmara's Voice Frequency Algorithm
import voice_frequency as rvf # Rudnik's Voice Frequency Algorithm

def are_close(first, second, threshold: float) -> bool:
    return abs(first - second) <= threshold;

def voice_frequency(path: str, proximity_treshold: float = 10, antiproximity_treshold: float = 200) -> float:
    ...
    multiples = False;
    for i in range(2,5):
        if are_close(pochmara * i, rudnik, proximity_treshold) or are_close(pochmara, rudnik * i, proximity_treshold):
            multiples = True;
            break;
    if multiples:
